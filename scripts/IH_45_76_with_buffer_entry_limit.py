def breach_low_buffer_time_filtered(df, SL=45.0, TP=76.0, buffer=5.0, cutoff_time=datetime.time(13, 45)):
    df = df.copy()
    ...
    df['entry_signal'] = df['iham'].shift(1) & (df['low'] < (prev_low - buffer))

    trades = []
    in_pos = False
    for ts, row in df.iterrows():
        if not in_pos and row['entry_signal'] and ts.time() < cutoff_time:
            entry_price = prev_low.loc[ts] - buffer if not np.isnan(prev_low.loc[ts]) else row['open']
