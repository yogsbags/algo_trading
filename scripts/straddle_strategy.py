        ...
        for day, day_data in df.groupby(df.timestamp.dt.date):
            ...
            for i in range(2, len(day_df)):
                ...
                # Handle VWAP crossings - Price crossing below VWAP
                    ...
                    # Don't automatically generate a short signal on VWAP crossing
                    # Let the LL-LH sequence form first
                ...
                if r.straddle_price > r.vwap and not in_trend:
                    ...
                    if not hh_hl_sequence_formed and highest_high is not None and highest_low is not None and \
                       highest_high > r.vwap and highest_low > r.vwap:
        ...
        try:
            ...
            for _, trade in trades_df.iterrows():
                ...
                exit_time = pd.to_datetime(trade['Exit Time']).strftime('%H:%M')
                 ...
                 # Handle strike_price which might be None
                strike_price = trade.get('strike_price')
                if strike_price is None or pd.isna(strike_price):
                    strike_price = '-'
                else:
                    strike_price = str(int(strike_price))  # Convert to string for formatting
                
                print(f"{str(trade['Date']):<12} {entry_time:<10} {exit_time:<10} "
                      f"{trade['Leg']:<6} {strike_price:<8} "
