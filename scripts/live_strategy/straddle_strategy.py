print("straddle_strategy.py is running")
...
class StraddleStrategy:
    ...
    def __init__(self, 
        ...
        # ATM strike for simulation mode
        ...
        try:
            ...
            if self.mode == 'simulate':
                if self.simulator:
                    ...
                    # Use provided exit_price or fallback to trade['price']
                    ...
                    # For EOD exits, ensure we use 15:20 as exit time
                    if exit_reason == 'EOD':
                        ...
                        eod_time = exit_time or datetime.now(self.ist_tz)
                    ...
                    trade.update({
                        ...
                        'points': points,
                        'status': 'closed',
                        'exit_time': eod_time if exit_reason == 'EOD' else (exit_time or datetime.now(self.ist_tz)),
                ...
                if trade['type'] == 'Short' and '|' in trade['symbol'] and '|' in trade['token']:
                    ...
                    if len(symbols) == 2 and len(tokens) == 2:
                        ...
                        # Place both orders
                        await self.broker.orders.place_order(call_order_params)
                        await self.broker.orders.place_order(put_order_params)
                    else:
    ...
    def get_weekly_expiry(self) -> str:
        """Get weekly expiry date (next Thursday) as DDMMMYY string"""
        from .utils import get_weekly_expiry
    ...
    def get_active_and_next_expiry(self) -> tuple:
        ...
        # Options expiry rules remain unchanged
    ...
    async def handle_signal(self, signal: str, timestamp: datetime, price: float):
        ...
        try:
            ...
            if signal == 'Long' or signal == 'Long1,Long2':
                ...
                if self.mode == 'live':
                    logger.info(f"[ORDER] Placing LONG orders: {self.futures_symbol}")
                    logger.info(f"[ORDER] Long1 params (fixed SL/TP): {order_params1}")
                    logger.info(f"[ORDER] Long2 params (trailing SL): {order_params2}")
                elif self.mode == 'simulate':
                    # Add both Long1 and Long2 as separate trades for metrics
                    trade1 = {
                        'type': 'Long1',
                        'entry_price': price,
                        'price': price,
                    ...
                    await self.broker.orders.place_order(order_params1)
                    await self.broker.orders.place_order(order_params2)
                # Long1: fixed SL/TP
                ...
                logger.info(f"[TRADE] Added Long1 trade to active_trades. Current count: {len(self.active_trades)}")
                
                # Long2: trailing target
                self.active_trades.append({
                    'type': 'Long2',
                    'symbol': self.futures_symbol,
                    'token': self.futures_token,
                ...
                logger.info(f"[TRADE] Added Long2 trade to active_trades. Current count: {len(self.active_trades)}")
                ...
                if self.mode == 'live':
                    ...
                    await self.broker.orders.place_order(order_params_call)
                    await self.broker.orders.place_order(order_params_put)
                self.active_trades.append({
                    'type': 'Short',
    ...
    async def process_instrument_setup(self):
        ...
        try:
            ...
            atm_strike = await self._get_atm_strike()
    ...
    def calculate_metrics(self) -> dict:
        ...
        wins = len([t for t in self.trade_history 
    ...
    async def run(self):
        if self.mode == 'simulate':
            ...
            options_expiry = self.get_weekly_expiry() if self.root_symbol == 'NIFTY' else self.get_monthly_expiry()
            ...
            expiry_for_xata = options_expiry
            ...
            while True:
                ...
                try:
                    ...
                    if hasattr(now, 'astimezone'):
                        ...
                        now_ist = now
                        now_ist_str = str(now)
                    call_price = candle['call_candle']['close']
                    put_price = candle['put_candle']['close']
            ...
            for i, row in df.iterrows():
                ...
                # Log candle data
                logger.info(f"[SIM] Historical data - Time: {now_ist_str} | Call Price: {row['call_open']:.2f} | Put Price: {row['put_open']:.2f} | Straddle: {round(row['straddle_price'], 2):.2f} | VWAP: {row['vwap']:.2f}")
                if (i+1) % 5 == 0:
        ...
        try:
            ...
            atm_strike = int(round(ltp / self.strike_interval) * self.strike_interval)
            ...
            while self.broker.quotes.is_market_open():
                ...
                if current_5min_boundary is None or boundary_time != current_5min_boundary:
                    if call_1min and put_1min:
                        ...
                        call_open = call_1min[0]['price']
                        call_high = max(x['price'] for x in call_1min)
                        call_low = min(x['price'] for x in call_1min)
                        call_close = call_1min[-1]['price']
                        call_volume = sum(x['volume'] for x in call_1min)
                        put_open = put_1min[0]['price']
                        put_high = max(x['price'] for x in put_1min)
                        put_low = min(x['price'] for x in put_1min)
                        put_close = put_1min[-1]['price']
                        put_volume = sum(x['volume'] for x in put_1min)
                        ...
                        logger.info(f"[LIVE][DEBUG] 5min candle at {candle_time.strftime('%H:%M')}: call_volume={call_volume}, put_volume={put_volume}, straddle_volume={straddle_volume}")
                        ...
                        try:
                            ...
                            if any(v is None or pd.isna(v) for v in [straddle_price, vwap, call_close, put_close]):
                                logger.warning(f"[LIVE] Invalid values - straddle_price: {straddle_price}, vwap: {vwap}, call_close: {call_close}, put_close: {put_close}")
                                continue
                            
                            if all(v is not None and not pd.isna(v) for v in [straddle_price, vwap, call_close, put_close]):
                                try:
                                    logger.info(f"[LIVE] 5min candle - Time: {candle_time.strftime('%Y-%m-%d %H:%M:%S')} | Straddle: {straddle_price:.2f} | VWAP: {vwap:.2f}")
                                    ...
                                    if signal:
                                        ...
                                        logger.info(f"Call Price: {call_close:.2f}")
                                        logger.info(f"Put Price: {put_close:.2f}")
                                ...
                                logger.warning("[LIVE] Invalid values detected in data, skipping signal processing")
                        except Exception as e:
                ...
                call_1min.append({'timestamp': now, 'price': call_price, 'volume': call_vol})
                put_1min.append({'timestamp': now, 'price': put_price, 'volume': put_vol})
...
async def main():
    ...
    try:
        ...
        if getattr(args, 'mode', None) == 'simulate' or getattr(args, 'start_date', None):
            ...
            if len(futures_df) > 0:
                first_fut_price = float(futures_df.iloc[0]['close'])
        ...
        strategy = StraddleStrategy(
            ...
            simulator=simulator,
            atm_strike=atm_strike if 'atm_strike' in locals() else None
        )
