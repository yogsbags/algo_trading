...
def parse_args():
    ...
    parser = argparse.ArgumentParser(description='Live Inverted Hammer Strategy')
...
class LiveInvertedHammerStrategy:
    ...
    def detect_inverted_hammer(self, candle: dict) -> bool:
        ...
        try:
            ...
            if is_ih:
                self.entry_price = low - self.buffer
                logger.info(f"✅ Inverted hammer detected!")
                logger.info(f"Entry price: {self.entry_price} (Low: {low} - Buffer: {self.buffer})")
                return True
            else:
                self.entry_price = None
    ...
    def calculate_metrics(self) -> dict:
        ...
        try:
            ...
            metrics = {
                ...
                'Net Profit (₹)': round(sum(trades_df['points']), 2),
            ...
            logger.info(f"Net Profit: ₹{metrics['Net Profit (₹)']}")
            logger.info(f"Total Points: {metrics['Total Points']}")
            logger.info(f"Average Points per Trade: {metrics['Average Points per Trade']}")
            logger.info(f"Best Trade: {metrics['Best Trade (Points)']} points")
            logger.info(f"Worst Trade: {metrics['Worst Trade (Points)']} points")
    ...
    def save_eod_results(self):
        ...
        try:
            ...
            results_dir = Path(project_root) / 'results' / 'inverted_hammer_buffer'
    ...
    async def run(self):
        ...
        try:
            ...
            while True:
                ...
                # Check cutoff time
                if now.time() > self.cutoff_time:
                    logger.info("Cutoff time passed. No new entries.")
                    break
...
async def main():
    try:
        ...
        strategy = LiveInvertedHammerStrategy(args)
        await strategy.run()
