...
def parse_args():
    ...
    parser.add_argument('--token', type=str, required=True,
                      help='Instrument token (e.g., 26009 for Bank Nifty, 57133 for Nifty)')
    ...
    parser.add_argument('--weekly-expiry', action='store_true',
                      help='Use weekly expiry instead of monthly expiry')
...
logger.info(f"Expiry type: {'Weekly' if CONFIG['INSTRUMENT']['weekly_expiry'] else 'Monthly'}")
...
class StrikeFinder:
    ...
    def get_all_dates_first_candle_close(self) -> pd.DataFrame:
        ...
        try:
            ...
            first_candles['strike_price'] = first_candles['close'].apply(self.round_to_strike)
    ...
    def get_active_and_next_expiry(self, target_date: date) -> str:
        ...
        if CONFIG['INSTRUMENT']['weekly_expiry']:
            # Get next Thursday for weekly expiry
            days_until_thursday = (3 - target_date.weekday()) % 7
            next_thursday = target_date + timedelta(days=days_until_thursday)
            
            # If the next Thursday is in the next month and today is not Thursday already,
            # we might need the current Thursday instead
            if next_thursday.month != target_date.month and days_until_thursday != 0:
                # Find this week's Thursday
                this_thursday = next_thursday - timedelta(days=7)
                # Use this Thursday if it's not in the past compared to target date
                if this_thursday >= target_date:
                    next_thursday = this_thursday
            
            # Format the weekly expiry (e.g., 08MAY25)
            expiry_str = next_thursday.strftime('%d%b%y').upper()
            logger.info(f"For date {target_date}, using weekly expiry: {expiry_str}")
        ...
        this_month_expiry = self.get_last_thursday(current_year, current_month)
            ...
            # Format expiry date with year (e.g., 30MAY25)
        expiry_str = this_month_expiry.strftime('%d%b%y').upper()
            logger.info(f"For date {target_date}, using monthly expiry: {expiry_str}")
