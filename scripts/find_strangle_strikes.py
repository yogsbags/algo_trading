...
def parse_args():
    ...
    parser.add_argument('--token', type=str, required=True,
                      help='Instrument token (e.g., 26009 for Bank Nifty, 26000 for Nifty)')
    ...
    parser.add_argument('--num-strikes', type=int, default=2,
...
logger.info(f"Number of strikes: {CONFIG['INSTRUMENT']['num_strikes']}")
...
class StrikeFinder:
    def __init__(self):
        ...
        self.num_strikes = CONFIG['INSTRUMENT']['num_strikes']
    ...
    def get_all_dates_first_candle_close(self) -> pd.DataFrame:
        """Get the close price of first 5min candle for all available dates"""
        try:
            # Read 5min data file
            ...
            if df['timestamp'].dt.tz is None:
                df['timestamp'] = df['timestamp'].dt.tz_localize(self.ist_timezone)
            else:
                df['timestamp'] = df['timestamp'].dt.tz_convert(self.ist_timezone)
            ...
            # Calculate initial ATM strike based on close price
            first_candles['atm_strike'] = first_candles['close'].apply(self.round_to_strike)
            
            # Generate initial strikes
            first_candles['strikes'] = first_candles.apply(
                lambda row: self.generate_strikes(row['close'], row['atm_strike']), 
                axis=1
            )
            
            # Explode the strikes list into separate rows
            result = first_candles.explode('strikes').reset_index(drop=True)
            result['strike_price'] = result['strikes']
            result = result[['date', 'strike_price']]
            
            logger.info(f"Found {len(result)} strikes across {len(first_candles)} days")
            logger.info(f"Date range: {result['date'].min()} to {result['date'].max()}")
            
            # Log the strikes for each day
            for date, group in result.groupby('date'):
                strikes = group['strike_price'].tolist()
                logger.info(f"Strikes for {date}: {strikes}")
            
            return result
            
        except Exception as e:
            logger.error(f"Error getting first candle closes: {str(e)}")
            raise

    def determine_atm_strike(self, current_price: float, call_price: float, put_price: float) -> int:
        """Determine ATM strike based on call/put price relationship"""
        # If call price > put price, strike is OTM, so ATM is higher
        if call_price > put_price:
            atm_strike = self.round_to_strike(current_price) + self.strike_interval
        # If call price < put price, strike is ITM, so ATM is lower
        elif call_price < put_price:
            atm_strike = self.round_to_strike(current_price) - self.strike_interval
        # If call price = put price, strike is ATM
        else:
            atm_strike = self.round_to_strike(current_price)
            
        logger.info(f"Determined ATM strike: {atm_strike} for price {current_price} (call: {call_price}, put: {put_price})")
        return atm_strike

    def generate_strikes(self, current_price: float, atm_strike: int) -> list:
        ...
        logger.info(f"Generated strikes for price {current_price}, ATM {atm_strike}: {strikes}")
        return strikes
    ...
    def get_active_and_next_expiry(self, target_date: date) -> str:
        ...
        # If target_date is after this month's expiry, use next month's expiry
        if target_date > this_month_expiry:
            # Move to next month
            if current_month == 12:
                next_month = 1
            ...
            next_month_expiry = self.get_last_thursday(next_year, next_month)
            expiry_str = next_month_expiry.strftime('%d%b%y').upper()
            logger.info(f"For date {target_date}, using expiry: {expiry_str} (next month)")
            return expiry_str
        else:
...
async def main():
    try:
        ...
        # Get first candle close prices for all dates
        data = finder.get_all_dates_first_candle_close()
